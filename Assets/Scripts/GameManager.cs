...
public class GameManager : MonoBehaviour {
	...
	//Initialise Modifiable Variables - they will be parameterised with an input file param:
    ...
    //added_to_script
    //Initialise list with the timings at which we pause the game
    public static int timeSaliva;
    public static float[] saliva_time;
    public static int saliva_time_counter = 0;
	...
	//initialise an empty array to store all inputs to the save() function  added_to_script
	public static object[] answer_array = new object[8];

	//Time given for likert scales added_to_script
	public static float timeLikert;

    //choose after how many trials participants are asked how stressed they feel
    public static int stress_likert_threshold; // added_to_script
    public static int stress_likert_counter = 0;  // added_to_script

    //Initialise variables - they will be parameterised with the input file param2
	...
	void InitGame(){
		...
		*/
		...
		if (escena == "SetUp") {
			...
			// keep track of the time elapsed on the likert screens
		}
		else if (escena == "ComplexityLikert")
		{
			boardScript.SetupScene(escena);

			tiempo = timeLikert;
			totalTime = timeLikert;
			showTimer = false;
		}
        else if (escena == "StressLikert")
        {
            boardScript.SetupScene(escena);
        ...
        else if (escena == "SalivaPause")
        {
            boardScript.SetupScene(escena);

            tiempo = timeSaliva;
            totalTime = timeSaliva;
            showTimer = true;
        }
        //end added_to_script
    ...
    void Update()
    {
        ...
        if (escena == "InterBlockRest" || escena == "InterTrialRest") //added to script
        {
            pauseManager();
        }
    }
    ...
    private void pauseManager()
    {
        ...
        {  //added to script
            ...
            if (Time.timeScale == 1)
            {
                saveTimeStamp("Unpause");   //added to script
                errorInScene("Pause");
            }
            else //added to script
            {
                saveTimeStamp("Pause");   //added to script
            }
        }
    }
    ...
    //edited this function to take additional inputs: the time spent on the complexity/stress likert screens and the option selected. stress values defaulted to 666 if not a stress trial
    public static void save(int answer, float timeSpent, int randomYes, string error, int complexAnswer,
		float complexTimeSpent, int stressAnswer, float stressTimeSpent) { //added_to_script
		...
		string dataTrialText = block + ";" + trial + ";" + answer + ";" + correct + ";" + timeSpent + ";" + randomYes
			+ ";" + instanceNum + ";" + xyCoordinates + ";" + error + ";" + complexAnswer + ";" + complexTimeSpent
...
//					}
			...
			dict.TryGetValue ("instance_id", out ksinstances[k-1].id);
			dict.TryGetValue ("ICdist_sampled", out ksinstances[k-1].type);
	...
	void assignVariables(Dictionary<string,string> dictionary){
		...
		string timeAnswerS;
		string timeLikertS; //added_to_script
        string stress_likert_thresholdS; //added_to_script
        string saliva_timeS; //added_to_script
        string timeSalivaS;	//added_to_script
		...
		dictionary.TryGetValue("timeLikert", out timeLikertS); //added_to_script

        dictionary.TryGetValue("stress_likert_threshold", out stress_likert_thresholdS); //added_to_script

        dictionary.TryGetValue("timeSaliva", out timeSalivaS); //added_to_script
		...
		timeLikert = Int32.Parse(timeLikertS); //added_to_script
        stress_likert_threshold = Int32.Parse(stress_likert_thresholdS); //added_to_script
        timeSaliva = Int32.Parse(timeSalivaS); //added_to_script
...
//		} else{
        ...
        //added_to_script
        dictionary.TryGetValue("saliva_time", out saliva_timeS);
        float[] temp_saliva_time = Array.ConvertAll(saliva_timeS.Substring(1, saliva_timeS.Length - 2).Split(','), float.Parse);
        saliva_time = new float[temp_saliva_time.Length];
        for (int i = 0; i < temp_saliva_time.Length; i++)
        {
            if (i == 0)
            {
                saliva_time[i] = temp_saliva_time[i] * 1000 * 60;
            }
            else
            {
                saliva_time[i] = saliva_time[i - 1] + (temp_saliva_time[i] * 1000 * 60);
            }
        }
...
//	}
	...
	public static void changeToNextScene(int answer, int randomYes){
		...
		if (escena == "SetUp") {
			saveHeaders ();
			Debug.Log("counter is" + stress_likert_counter);
			Debug.Log("threshold is " + stress_likert_threshold);
			...
			if (answer == 2) {
			//start added_to_script
			// save answers, including the additional fields of time spent on likert screens and likert selections
				answer_array[0] = answer;
				answer_array[1] = timeQuestion;
				answer_array[2] = randomYes;
				answer_array[3] = "";

				saveTimeStamp("ParticipantTimeOut");
			} else {
				answer_array[0] = answer;
				answer_array[1] = timeAnswer - tiempo;
				answer_array[2] = randomYes;
				answer_array[3] = "";

				saveTimeStamp("ParticipantAnswer");
			}
			SceneManager.LoadScene("ComplexityLikert");
		}
		else if (escena == "ComplexityLikert")
		{
            stress_likert_counter += 1;
			Debug.Log("counter is" + stress_likert_counter);

            if (stress_likert_counter == stress_likert_threshold)
            {
				answer_array[4] = answer;
				answer_array[5] = timeLikert - tiempo;
				SceneManager.LoadScene("StressLikert");
            }
            else
            {
				answer_array[4] = answer;
				answer_array[5] = timeLikert - tiempo;
				answer_array[6] = 666; // default value if there is no stress likert
				answer_array[7] = 666f; // default value if there is no stress likert

				save((int) answer_array[0], (float) answer_array[1], (int) answer_array[2], (string) answer_array[3],
					(int) answer_array[4], (float) answer_array[5], (int) answer_array[6], (float) answer_array[7]);

                // if we haven't hit the last break yet
                if (saliva_time_counter < saliva_time.Length)
                {
                    // if it's time for saliva, go to saliva pause
                    if (stopWatch.ElapsedMilliseconds > saliva_time[saliva_time_counter])
                    {
                        Debug.Log("time stamp is " + stopWatch.ElapsedMilliseconds);
                        Debug.Log("the saliva counter is" + saliva_time_counter);
                        Debug.Log("the saliva time is" + saliva_time[saliva_time_counter]);

                        saliva_time_counter += 1;
                        SceneManager.LoadScene("SalivaPause");
                    }
                    // if it's not proceed to ITR
                    else
                    {
                        SceneManager.LoadScene("InterTrialRest");
                    }
                }
                // if we've passed the last break
                else
                {
                    SceneManager.LoadScene("InterTrialRest");
                }
        ...
        {
            stress_likert_counter = 0;
			Debug.Log("counter is" + stress_likert_counter);

			answer_array[6] = answer;
			answer_array[7] = timeLikert - tiempo;
	...
	public static void errorInScene(string errorDetails){
		...
		int answer = 3;
		int randomYes = -1;
		save (answer, timeQuestion, randomYes, errorDetails, 666, 666, 666, 666);
		changeToNextTrial ();
	}
